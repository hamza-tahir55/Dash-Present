...
export class DashboardApi {
  ...
  static async getPresentations(): Promise<PresentationResponse[]> {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/presentation/all`,
        {
          method: "GET",
        }
      );
  ...
  static async getPresentation(id: string) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/presentation?id=${id}`,
        {
          method: "GET",
        }
      );
  ...
  static async deletePresentation(presentation_id: string) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/presentation?id=${presentation_id}`,
        {
          method: "DELETE",
          headers: getHeader(),
        }
