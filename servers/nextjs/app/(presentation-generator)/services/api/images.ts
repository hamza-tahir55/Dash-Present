...
export class ImagesApi {
 ...
 static async uploadImage(file: File): Promise<ImageAssetResponse> {
    ...
    const response = await fetch(`http://127.0.0.1:8000/api/v1/ppt/images/upload-image`, {
      method: "POST",
  ...
  static async getUploadedImages(): Promise<ImageAssetResponse[]> {
    try {
    const response = await fetch(`http://127.0.0.1:8000/api/v1/ppt/images/uploaded`);
  ...
  static async deleteImage(image_id: string): Promise<{success: boolean, message?: string}> {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/v1/ppt/images/uploaded-image/${image_id}`, {
        method: "DELETE"
      });
