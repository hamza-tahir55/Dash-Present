...
export class PresentationGenerationApi {
  static async uploadDoc(documents: File[]) {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/files/upload`,
        {
          method: "POST",
  ...
  static async decomposeDocuments(documentKeys: string[]) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/files/decompose`,
        {
          method: "POST",
          headers: getHeader(),
          body: JSON.stringify({
  ...
  static async editSlide(
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/slide/edit`,
        {
          method: "POST",
          headers: getHeader(),
          body: JSON.stringify({
  ...
  static async updatePresentationContent(body: any) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/presentation/update`,
        {
          method: "PUT",
          headers: getHeader(),
  ...
  static async generateImage(imageGenerate: ImageGenerate) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/images/generate?prompt=${imageGenerate.prompt}`,
        {
          method: "GET",
          headers: getHeader(),
          cache: "no-cache",
  ...
  static getPreviousGeneratedImages = async (): Promise<PreviousGeneratedImagesResponse[]> => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/images/generated`,
        {
          method: "GET",
          headers: getHeader(),
        }
  ...
  static async searchIcons(iconSearch: IconSearch) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/icons/search?query=${iconSearch.query}&limit=${iconSearch.limit}`,
        {
          method: "GET",
          headers: getHeader(),
          cache: "no-cache",
  ...
  static async exportAsPPTX(presentationData: any) {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ppt/presentation/export/pptx`,
        {
          method: "POST",
          headers: getHeader(),
          body: JSON.stringify(presentationData),
