...
const Header = ({
  ...
  const downloadLink = (path: string) => {
    try {
      // Ensure the path is properly decoded for display but encoded for URL usage
      ...
      if (window.opener) {
        ...
        // Use fetch to get the file as a blob to avoid network errors with special characters
        fetch(path)
          .then(response => {
            if (!response.ok) {
      ...
      console.error('Error in downloadLink:', error);
      toast.error("Failed to process download", {
        description: "There was a problem with the file download. Please try again.",
      });
    }
  };
