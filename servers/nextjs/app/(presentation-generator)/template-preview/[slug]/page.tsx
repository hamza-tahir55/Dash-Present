...
const GroupLayoutPreview = () => {
  ...
  useEffect(() => {
    const loadCustomLayouts = async () => {
      if (!isCustom) return;
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/v1/ppt/template-management/get-templates/${presentationId}`);
        if (!res.ok) return;
  ...
  const deleteLayouts = async () => {
    ...
    refetch();
    router.back();
    const response = await fetch(`http://127.0.0.1:8000/api/v1/ppt/template-management/delete-templates/${presentationId}`, {
      method: "DELETE",
    }); 
    if (response.ok) {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/v1/ppt/template-management/save-templates`, {
        method: "POST",
