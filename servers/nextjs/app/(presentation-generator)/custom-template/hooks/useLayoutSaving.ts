...
export const useLayoutSaving = (
  ...
  const convertSlideToReact = async (slide: ProcessedSlide, presentationId: string, FontUrls: string[]) => {
    ...
    while (retryCount < maxRetries) {
      try {
        const response = await fetch("http://127.0.0.1:8000/api/v1/ppt/html-to-react/", {
          method: "POST",
          headers: {
  ...
  const saveLayout = useCallback(async (layoutName: string, description: string): Promise<string | null> => {
    ...
    try {
      ...
      await fetch("http://127.0.0.1:8000/api/v1/ppt/template-management/templates", {
        method: "POST",
      ...
      const saveResponse = await fetch(
        "http://127.0.0.1:8000/api/v1/ppt/template-management/save-templates",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
