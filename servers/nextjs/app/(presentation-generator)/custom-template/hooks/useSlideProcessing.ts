...
export const useSlideProcessing = (
  ...
  const processSlideToHtml = useCallback(
    async (slide: SlideData, index: number) => {
      ...
      try {
        const htmlResponse = await fetch("http://127.0.0.1:8000/api/v1/ppt/slide-to-html/", {
          method: "POST",
          headers: {
  ...
  const processFile = useCallback(async () => {
    ...
    try {
      ...
      if (isPdf) {
        ...
        const pdfResponse = await fetch("http://127.0.0.1:8000/api/v1/ppt/pdf-slides/process", {
          method: "POST",
          body: formData,
        });
        ...
        const pptxResponse = await fetch("http://127.0.0.1:8000/api/v1/ppt/pptx-slides/process", {
          method: "POST",
          body: formData,
        });
        slidesResponseData = await ApiResponseHandler.handleResponse(
